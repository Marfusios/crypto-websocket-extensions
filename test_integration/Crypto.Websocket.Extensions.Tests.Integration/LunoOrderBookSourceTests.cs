...
namespace Crypto.Websocket.Extensions.Tests.Integration
{
    public class LunoOrderBookSourceTests(ITestOutputHelper testOutputHelper)
    {
        ...
        public async Task StreamingRecordedMessages_ShouldHandleOrderBookCorrectly()
        {
            ...
            source.BufferInterval = TimeSpan.FromMilliseconds(2);
            using var orderBook = new CryptoOrderBook(pair, source);
            orderBook.NotifyForLevelAndAbove = topLevelCount;
                ...
                .Subscribe(info =>
                {
                    try
                    {
                        ...
                        if (_updateMessages.Count > 2 && _topNUpdateMessages.Last() != _updateMessages.Last())
                            testOutputHelper.WriteLine($"Websocket updates are out of sync: {info.ServerSequence}.");
                        ...
                        if (lastHeartbeatAt.Subtract(_timestamp) > TimeSpan.FromMilliseconds(300))
                            testOutputHelper.WriteLine($"""
                                                        Orderbook hasn't changed for more than 300 milliseconds: {info.ServerSequence}.
                            ...
                            testOutputHelper.WriteLine($"""
                                                        Orderbook hasn't changed for more than 3 seconds: {info.ServerSequence}.
                                                        Ask: {info.Quotes.AskAmount,8} @ {info.Quotes.Ask,-8}
                                                        Bid: {info.Quotes.BidAmount,8} @ {info.Quotes.Bid,-8}
                                                        """);
                    ...
                    {
                        testOutputHelper.WriteLine($"Error handling message: {info.ServerSequence}. {exception}");
                    }
                });
