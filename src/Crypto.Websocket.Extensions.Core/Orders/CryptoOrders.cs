...
namespace Crypto.Websocket.Extensions.Core.Orders
{
    ...
    public class CryptoOrders : ICryptoOrders
    {
        ...
        CryptoOrder ICryptoOrders.FindActiveOrderByClientId(string clientId)
        {
        ...
        private void HandleOrderUpdated(CryptoOrder order)
        {
            ...
            if (order.OrderStatus == CryptoOrderStatus.Executed ||
                order.OrderStatus == CryptoOrderStatus.PartiallyFilled)
            {
