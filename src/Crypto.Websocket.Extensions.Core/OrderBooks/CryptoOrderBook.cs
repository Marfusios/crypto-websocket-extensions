...
namespace Crypto.Websocket.Extensions.Core.OrderBooks
{
    ...
    public class CryptoOrderBook : ICryptoOrderBook
    {
        ...
        private readonly OrderBookSide _bidLevels = new OrderBookSide(CryptoOrderSide.Bid);
        private readonly OrderBookSide _askLevels = new OrderBookSide(CryptoOrderSide.Ask);
        ...
        public OrderBookLevel FindBidLevelByPrice(double price)
        {
            lock (_locker)
            {
                return _bidLevels.GetByPriceFirst(price);
            }
        }
        ...
        public OrderBookLevel[] FindBidLevelsByPrice(double price)
        {
            lock (_locker)
            {
                return _bidLevels.GetByPrice(price);
            }
        }
        ...
        public OrderBookLevel FindAskLevelByPrice(double price)
        {
            lock (_locker)
            {
                return _askLevels.GetByPriceFirst(price);
            }
        }
        ...
        public OrderBookLevel[] FindAskLevelsByPrice(double price)
        {
            lock (_locker)
            {
                return _askLevels.GetByPrice(price);
            }
        }
        ...
        private void HandleSnapshot(OrderBookLevel[] levels)
        {
            ...
            foreach (var level in levels)
            {
                ...
                {
                    _bidLevels.Add(level);
                }


                if (level.Side == CryptoOrderSide.Ask)
                {
                    _askLevels.Add(level);
                }
            }
        ...
        private void UpdateLevels(OrderBookLevel[] levels)
        {
            foreach (var level in levels)
            {
                ...
                InsertToCollection(collection, existing, previousPrice, previousAmount);
            }
        }

        private void InsertToCollection(OrderBookSide collection, 
            OrderBookLevel level, double? previousPrice = null, double? previousAmount = null)
        {
            ...
            {
                collection.UpdateOrAdd(level, previousPrice, previousAmount);
                return;
            }
        ...
        {
            var bid = _bidLevels.Root?.Data;
            var ask = _askLevels.Root?.Data;
        ...
        private OrderBookLevel[] ComputeAllLevels()
        {
            lock (_locker)
            {
                return _bidLevels.LevelIndex.Concat(_askLevels.LevelIndex)
                    .Select(x => x.Value.Data)
                    .ToArray();
            }
        }

        private IReadOnlyDictionary<double, OrderBookLevel[]> ComputeLevelsPerPrice(OrderBookSide levels)
        {
            lock (_locker)
            {
                return levels
                    .GetAllGroupedByPrice();
            }
        }

        private OrderBookSide GetLevelsCollection(CryptoOrderSide side)
        {
            if (side == CryptoOrderSide.Undefined)
                return null;
        ...
        private OrderBookLeafsById GetAllCollection(CryptoOrderSide side)
        {
            if (side == CryptoOrderSide.Undefined)
                return null;
            return side == CryptoOrderSide.Bid ? 
                _bidLevels.LevelIndex : 
                _askLevels.LevelIndex;
        }
